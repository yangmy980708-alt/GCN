...
def eval_unassembled_resjac_claw_dg(U,transf_data,elem,elem_data,ldof2gdof_var,e2e):
	...
	for e in range(nelem):
		Ue= torch.tensor(U[ldof2gdof_var[:,e]]).reshape(-1, 1).double().to('cuda')
		# Uef[:,0]=Ue.reshape(Uef[:,0].shape,order='F')

		Uef[:,0] = ReshapeFix(Ue,[nvar_per_elem,1],order='F')
		# Uef[:, 0] = Ue
		for f in range(nf):
		...
		Re.append(ReshapeFix((Re0_ + Re1_ + Re2_), [neqn_per_elem, 1], order='F'))
		dRe.append(ReshapeFix((dRe0_ + dRe1_ + dRe2_[:,:,0]), [neqn_per_elem, nvar_per_elem, 1], order='C'))
		"在有限元解时候的Re dRe"
...
def create_fem_resjac(fespc,Uf,transf_data,elem,elem_data,
	...
	if fespc=='dg' or fespc=='DG':
		...
		raise ValueError('FE space only support dg!')
...
def intg_elem_claw_intface(Uef,transf_data,elem,elem_data,e2e,e,elst):
	...
	for f in range(nf):
		...
		for k in range(nqf):
			...
			Re = Re + ReshapeFix(wf[k, f] * torch.mm(TvfI0,Fs), Re.shape, order='F')
			dRe[:, :, 0] = dRe[:, :, 0] + wf[k, f] * torch.mm(TvfI0, torch.mm(dFsdUQI, TvfI.T))
			dRe[:,:,1+f] = dRe[:,:,1+f]+wf[k,f] * torch.mm(TvfI0, torch.mm(dFsdUQO, TvfO.T))
	return Re, dRe
...
def solve_fem_GCNN(DataLoader,LossF,model,tol=1e-3,maxit=2000,qoiidx=None,softidx=None,penaltyConstant=None):
	"""Wrapper""" 
	startime=time.time()
	model,info,erlist=solve_SGD(DataLoader,LossF,model,tol,maxit,qoiidx,softidx,penaltyConstant)
	print('wallclock time of all epochs = ',time.time()-startime)
	return model, info,erlist
...
def solve_SGD(DataLoader,LossF,model,tol,maxit,qoiidx,softidx,penaltyConstant,plotFlag='True'):
	...
	for epoch in range(maxit):
		print('epoch = ',epoch)
		startime=time.time()
		er,loss,model,erlist=trainmodel(DataLoader,LossF,model,
		...
		Er.append(er);Loss.append(loss);Erlist.append(erlist)
	...
	return model,{"Er":np.asarray(Er),"Loss":np.asarray(Loss)},erlist
...
def trainmodel(DataLoader,LossF,model,optimizer,criterion,qoiidx,softidx,penaltyConstant):
	...
	return er_0/len(DataLoader),loss.norm().item()/len(DataLoader),model,erlist
