...
def solve_GCNN_MBGD(number_of_unknowns_Darcy, number_of_FE_nodes_u, number_of_FE_nodes_p,DataLoader,LossF,model,tol=1e-3,maxit=2000,qoiidx=None,softidx=None,penaltyConstant=None):
	"""Wrapper"""
	startime=time.time()
...
def solve_SGD(number_of_unknowns_Darcy, number_of_FE_nodes_u, number_of_FE_nodes_p,
    ...
    optimizer = torch.optim.SGD(
        model.parameters(),
        lr=0.01,  # 学习率通常比 Adam 更大
        momentum=0,  # 禁用动量以保证纯 MBGD 行为
        weight_decay=0  # 无权重衰减
    )
    criterion = torch.nn.MSELoss()

    # 初始化存储变量
    Er_u = []; Er_u1 = []; Er_u2 = []; Er_p = []; Er_D = []; Loss = []
