...
class interface_Chebnet(torch.nn.Module):
	def __init__(self,split):
		super(interface_Chebnet, self).__init__()
		nci=2;nco=1
	...
	def forward(self, data):
		...
		n3=self.split[2]#NS_U 节点数
		n4=self.split[3]#NS_p节点数
		n5=self.split[4]#NS单元数
		idx1=[i for i in range(n1)]
		idx2=[n1+i for i in range(n3)]
		idx3=[n1+n3+i for i in range(n3)]
		idx4=[n1+2*n3+i for i in range(n4)]
		x1=x[idx1,:]
		x2=x[idx2,:]
		x3=x[idx3,:]
		x4=x[idx4,:]
		edge_index1=edge_index[:,0:n2]
		edge_index2=edge_index[:,n2:n2+n5]
		edge_index3=edge_index[:,n2+n5:n2+2*n5]
		edge_index4=edge_index[:,n2+2*n5:n2+3*n5]
		...
		return torch.cat([x1, uv,x4],axis=0)#F.log_softmax(x, dim=1)
