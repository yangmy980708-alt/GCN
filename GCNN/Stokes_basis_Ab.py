...
def assemble_matrix_from_volume_integral_triangle_global(coefficient_function, M_partition, T_partition, M_basis_trial, T_basis_trial, T_basis_test,
    ...
    for alpha in range(number_of_trial_local_basis):
           for beta in range(number_of_test_local_basis):
               ...
               for i in range(Gauss_point_number):
                   ...
                   temp += Gauss_coefficient_local_triangle[i,:] * coef * trial_basis_value * test_basis_value # * u_j
...
def assemble_fixed_matrix_vector_for_Stokes_Taylor_Hood(M_partition, T_partition, M_basis_trial_u, T_basis_trial_u,
                                                        T_basis_test_u, T_basis_p):
    coef_data = Functions_torch()

    A1 = assemble_matrix_from_volume_integral_triangle_global(getattr(coef_data, 'function_nu'), M_partition, T_partition, M_basis_trial_u, T_basis_trial_u,
                                                        T_basis_test_u, 2, 1, 0, 2, 1, 0)
    A2 = assemble_matrix_from_volume_integral_triangle_global(getattr(coef_data, 'function_nu'), M_partition, T_partition, M_basis_trial_u, T_basis_trial_u,
                                                        T_basis_test_u, 2, 0, 1, 2, 0, 1)
    A3 = assemble_matrix_from_volume_integral_triangle_global(getattr(coef_data, 'function_nu'), M_partition, T_partition, M_basis_trial_u, T_basis_trial_u,
                                                        T_basis_test_u, 2, 1, 0, 2, 0, 1)
    A4 = assemble_matrix_from_volume_integral_triangle_global(getattr(coef_data, 'function_nu'), M_partition, T_partition, M_basis_trial_u, T_basis_trial_u,
    ...
    b = torch.cat([b1, b2, temp_b], dim=0)
    b = Double(b)
    return A, b
...
def Gauss_quadrature_for_line_integral_trial_test_triangle(
    ...
    if end_point_1[1] == end_point_2[1]:  # The line is horizontal
        ...
        for i in range(Gpn):
            ...
            r += Gauss_coefficient_local_1D[i] * coef * r_1 * r_2
        ...
        for i in range(Gpn):
            ...
            r_2 = triangular_local_basis_element(
                    ...
                    test_derivative_degree_x, test_derivative_degree_y)
            r += Gauss_coefficient_local_1D[i] * Jacobi * coef * r_1 * r_2
    r = Double(r)
    return r
...
def assemble_matrix_from_volume_integral_FE_triangle(uh, M_partition, T_partition,
    ...
    for alpha in range(number_of_trial_local_basis):
        for beta in range(number_of_test_local_basis):
            ...
            #                     (T_basis_test[beta, :].cpu().numpy(), T_basis_trial[alpha, :].cpu().numpy())),
            #                     shape=(Nb_test, Nb_trial))
            A += sp.coo_matrix((temp.detach().cpu().numpy(),
                                (T_basis_test[beta, :].cpu().numpy(), T_basis_trial[alpha, :].cpu().numpy())),
                               shape=(Nb_test, Nb_trial))
            r = A.toarray()
            r = Double(r)
...
def assemble_vector_from_volume_integral_2FE_triangle(uh_FE1, uh_FE2, M_partition,
    ...
    for beta in range(int(T_basis_test.shape[0])):
        ...
        for i in range(Gauss_point_number):
            ...
            row_indices = T_basis_test[beta, :].to(device)
            col_indices = torch.zeros_like(row_indices, device=device)  # 保持 GPU 计算
        R += sp.coo_matrix((temp.detach().cpu().numpy(),
                            (row_indices.cpu().numpy(), col_indices.cpu().numpy())),
                           shape=(vector_size, 1))
        r = R.toarray()
        r = Double(r)
