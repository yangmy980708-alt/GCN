...
from GCNN.ModelDarcy import Darcy_Chebnet
from GCNN.ModelStokes import NS_Chebnet
...
from GCNN.generate_residual_SD_D import generate_residual_D
from GCNN.generate_residual_SD_u import generate_residual_u
from GCNN.generate_residual_SD_face import generate_residual_interface
from GCNN.solve_GCNN_Darcy import solve_GCNN_Darcy
from GCNN.solve_GCNN_Stokes import solve_GCNN_Stokes
# from GCNN.solve_GCNN_loss_tol_strong import solve_GCNN
# from GCNN.solve_GCNN_MBGD import solve_GCNN_MBGD
sys.path.insert(0, os.path.abspath('../demo_Stokes_Darcy'))
from demo_Stokes_Darcy.Stokes_Darcy_fem import steady_Stokes_Darcy_fem

left_S=0;right_S=1;bottom_S=-0.25;top_S=0
left_D=0;right_D=1;bottom_D=0;top_D=0.75
h_partition_S=(1/4,1/4);h_partition_D=(1/4,1/4)
...
phih, uh1, uh2, ph ,r_D_boundary, r_Stokes_boundary,boundary_nodes_D, boundary_nodes_S\
	= steady_Stokes_Darcy_fem(left_S, right_S, bottom_S, top_S,
...
'''真解'''
coef_data_torch = Functions_torch()
phih_D = coef_data_torch.phi_exact_solution(Double(M_basis_phi[0,:]), Double(M_basis_phi[1,:]))
...
u_2 = coef_data_torch.u2_exact_solution(Double(M_basis_u[0,:]), Double(M_basis_u[1,:]))
u_2 = u_2.cpu().numpy().reshape(-1, 1)
p = np.zeros((number_of_FE_nodes_p,1))
...
connectivity_Stokes = torch.cat([connectivity_uv, connectivity_uv,
							  connectivity_p], axis=1)
#此处写入残差
LossF_D = []
fcn_D = lambda u_:generate_residual_D(u_,
    left_S, right_S, bottom_S, top_S,
    left_D, right_D, bottom_D, top_D,
    h_partition_S, h_partition_D,
    fix_pressure, Dirichlet_switch,
    Darcy_scaling_constant,U_boundary_D, U_boundary_S)#使用真解投入求解区域边界
LossF_D.append(fcn_D)

LossF_u = []
fcn_u = lambda u_:generate_residual_u(u_,
    left_S, right_S, bottom_S, top_S,
    left_D, right_D, bottom_D, top_D,
    h_partition_S, h_partition_D,
    fix_pressure, Dirichlet_switch,
    Darcy_scaling_constant,U_boundary_D, U_boundary_S)#使用真解投入求解区域边界
LossF_u.append(fcn_u)

LossF_interface = []
fcn_interface = lambda u_:generate_residual_interface(u_,
    left_S, right_S, bottom_S, top_S,
    left_D, right_D, bottom_D, top_D,
    h_partition_S, h_partition_D,
    fix_pressure, Dirichlet_switch,
    Darcy_scaling_constant,U_boundary_D, U_boundary_S)#使用真解投入求解区域边界
LossF_interface.append(fcn_interface)

LossF_Darcy = LossF_D + LossF_interface
LossF_Stokes = LossF_u + LossF_interface


ii=0
Graph_D=[]
# Ue=Double(U_fem.flatten().reshape(-1,1))#fem解训练
Ue_D=Double(phih_D.flatten().reshape(-1,1))
fcn_id_D=Double(np.asarray([ii]).reshape(-1,1))  # 将 fcn_idx 转换为形状为 (1, 1) 的张量
Ue_aug_D=torch.cat((fcn_id_D,Ue_D),axis=0)

analyU_Stokes = np.vstack((u_1, u_2,p))
Graph_Stokes=[]
# Ue=Double(U_fem.flatten().reshape(-1,1))#fem解训练
Ue_Stokes=Double(analyU_Stokes.flatten().reshape(-1,1))
fcn_id_Stokes=Double(np.asarray([ii]).reshape(-1,1))  # 将 fcn_idx 转换为形状为 (1, 1) 的张量
Ue_aug_Stokes=torch.cat((fcn_id_Stokes,Ue_Stokes),axis=0)
xcg_gcnn_D=np.zeros((2,M_basis_phi.shape[1]))
xcg_gcnn_Stokes=np.zeros((2,2*M_basis_u.shape[1] + M_basis_p.shape[1]))
# 填充 M_basis_phi 的坐标
for i in range(M_basis_phi.shape[1]):
    xcg_gcnn_D[:, i] = M_basis_phi[:, i]
# 填充 M_basis_u 的坐标
for i in range(M_basis_u.shape[1]):
    xcg_gcnn_Stokes[:,i] = M_basis_u[:, i]
    xcg_gcnn_Stokes[:,M_basis_u.shape[1] + i] = M_basis_u[:, i]
# 填充 M_basis_p 的坐标
for i in range(M_basis_p.shape[1]):
    xcg_gcnn_Stokes[:, 2 * M_basis_u.shape[1] + i] = M_basis_p[:, i]

'''Darcy Graph '''
Uin_D=Double(xcg_gcnn_D.T)
graph_D=Data(x=Uin_D,y=Ue_aug_D,edge_index=connectivity_phi)
Graph_D.append(graph_D)
DataList_D=[[Graph_D[0]]]
TrainDataloader_D=DataLoader(DataList_D,batch_size=1)
'''Stokes'''
Uin_Stokes=Double(xcg_gcnn_Stokes.T)
graph_Stokes=Data(x=Uin_Stokes,y=Ue_aug_Stokes,edge_index=connectivity_Stokes)
Graph_Stokes.append(graph_Stokes)
DataList_Stokes=[[Graph_Stokes[0]]]
TrainDataloader_Stokes=DataLoader(DataList_Stokes,batch_size=1)
device=torch.device('cuda' if torch.cuda.is_available() else 'cpu')
split_D=[M_basis_phi.shape[1],connectivity_phi.shape[1]]
model_D=Darcy_Chebnet(split_D).to(device)
model_D = model_D.double()
split_Stokes=[M_basis_u.shape[1], M_basis_p.shape[1], connectivity_uv.shape[1]]
model_Stokes = NS_Chebnet(split_Stokes).to(device)
model_Stokes = model_Stokes.double()

# model = model_D + model_Stokes#这一步不确定是否可以这样相加，还是全部份区域进行计算

tol=1e-08
maxit_1=3
maxit_2=3
[model_D,LossF_Darcy,Er_D,Erlist_D,erlist_D] = \
                                  solve_GCNN_Darcy(number_of_unknowns_Darcy,number_of_FE_nodes_u,
                                             number_of_FE_nodes_p,TrainDataloader_D,
                                             LossF_Darcy, model_D, tol, maxit_1)#Darcy

[model_Stokes,LossF_Stokes,Er_u,Er_u1,Er_u2,Er_p,Erlist_u,Erlist_u1,Erlist_u2,Erlist_p,erlist_u, erlist_u1, erlist_u2, erlist_p] = \
                                  solve_GCNN_Stokes(number_of_unknowns_Darcy,number_of_FE_nodes_u,
                                             number_of_FE_nodes_p,TrainDataloader_Stokes,
                                             LossF_Stokes, model_Stokes, tol, maxit_2)
torch.save(model_D, './Model_D.pth')
solution_D = model_D(Graph_D[0].to('cuda'))
solution_D = ReshapeFix(torch.clone(solution_D), [len(solution_D.flatten()), 1], 'C')
solution_D = solution_D.detach().cpu().numpy()
phih_GCNN = solution_D

torch.save(model_Stokes, './Model_Stokes.pth')
solution_Stokes = model_Stokes(Graph_Stokes[0].to('cuda'))
print('wallclock time = ',time.time()-startime)
solution_Stokes = ReshapeFix(torch.clone(solution_D), [len(solution_D.flatten()), 1], 'C')
solution_Stokes = solution_Stokes.detach().cpu().numpy()
uh1_GCNN = solution_Stokes[:number_of_FE_nodes_u]
uh2_GCNN = solution_Stokes[number_of_FE_nodes_u:2 * number_of_FE_nodes_u]
ph_GCNN = solution_Stokes[2 * number_of_FE_nodes_u: 2*number_of_FE_nodes_u+number_of_FE_nodes_p]
uh_GCNN = np.sqrt(uh1_GCNN**2 + uh2_GCNN**2)
savemat('solution_D.mat',{'solution_D':solution_D})
savemat('solution_Stokes.mat',{'solution_Stokes':solution_Stokes})
...
print('LossF_Darcy=',LossF_Darcy[-1])
print('LossF_Stokes=',LossF_Stokes[-1])
savemat('LossF_Stokes.mat',{'LossF_Stokes':LossF_Stokes})
savemat('LossF_Darcy.mat',{'LossF_Darcy':LossF_Darcy})
