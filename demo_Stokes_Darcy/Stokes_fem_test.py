...
if Dirichlet_switch == 1:
    [A, b] = treat_Dirichlet_boundary_Stokes_with_Darcy(getattr(coef_data, 'function_g1_Stokes'), getattr(coef_data, 'function_g2_Stokes'), A, b, boundary_nodes_S, M_basis_u, number_of_FE_nodes_u, number_of_unknowns_Darcy)

number_of_unknows_before_p = 2 * number_of_FE_nodes_u + number_of_unknowns_Darcy
fixed_p_index = 1
[A, b] = fix_pressure_Stokes(getattr(coef_data,'function_fix_p'), fix_pressure, A, b, number_of_unknows_before_p, fixed_p_index,
                                 M_basis_p)
