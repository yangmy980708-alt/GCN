...
# 							   mltidx_from_linidx
from pycamotk.pyCaMOtk.check import is_type
from pycamotk.pyCaMOtk.ndist_mltdim import ndist_mltdim_hcube, ndist_mltdim_simp
from pycamotk.pyCaMOtk.tens_core import linidx_from_mltidx, tensprod_vector_unif,\
							   mltidx_from_linidx
...
def create_geom_mltdim_simplex(ndim,porder,ndist0):
	nnpd=porder+1;
	nf=ndim+1
	zk=ndist_mltdim_simp(ndim,nnpd,ndist0)#调用函数 ndist_mltdim_simp 来获取参考元素节点的坐标，返回值存储在 zk 中。
