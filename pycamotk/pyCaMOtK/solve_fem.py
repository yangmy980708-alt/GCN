...
from pycamotk.pyCaMOtk.create_fem_resjac import create_fem_resjac,create_fem_resjac_f
...
def solve_fem_f(fespc, transf_data, elem,
			  elem_data,ldof2gdof_eqn,
			  ldof2gdof_var, e2e, spmat,
			  dbc,Uf0,tol,maxit):
	if dbc is None:
	...
	if maxit is None:
		maxit=10
	fcn=lambda u_:create_fem_resjac_f(fespc,u_,transf_data,elem,elem_data,ldof2gdof_eqn,ldof2gdof_var,e2e,spmat,dbc)
	# Han Gao create this for test, delete it after all finished
	Uf,info=solve_newtraph_HanGaoTemp(fcn,Uf0,tol,maxit)
	U=np.zeros(ndof_var)
	if len(dbc_idx)==0:
