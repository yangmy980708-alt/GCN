...
import numpy as np
import pdb
from pyCaMOtk.intg_elem_claw_vol import intg_elem_claw_vol,intg_elem_claw_vol_f
from pyCaMOtk.intg_elem_claw_extface import intg_elem_claw_extface
################################################################################

def eval_unassembled_resjac_claw_cg_f_no_tb(U,transf_data,elem,elem_data,ldof2gdof_var):
	...
	for e in range(nelem):
		...
		Re[:,e]=Re0_.reshape(Re[:,e].shape,order='F')
		dRe[:,:,e]=dRe0_.reshape(dRe[:,:,e].shape,order='F')
	return Re,dRe