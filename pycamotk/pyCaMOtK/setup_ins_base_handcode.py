...
# 	dUb[:,:,0]=np.eye(nvar)
# 	Fn=-pars[-ndim-1:]#ndim=2 此行代码的意义为pars最后三个元素
...
class IncompressibleNavierStokes_f(object):
	...
	def __init__(self,ndim):
		...
		self.nvar=ndim+1
		self.srcflux=lambda UQ,f,pars,x:\
		             eval_ins_base_handcode_srcflux_f(UQ,f,pars,x)
		self.bndstvcflux=lambda nbcnbr,UQ,pars,x,n:\
					     eval_ins_base_handcode_bndstvc_intr_bndflux_pars_f(UQ,pars,x,n)

def eval_ins_base_handcode_srcflux_f(UQ,f,pars,x):
	u=UQ[:,0]; q=UQ[:,1:]
	ndim=u.shape[0]-1
	neqn=ndim+1
	ncomp=ndim+1
	...
	S=np.vstack([-rho*dv.dot(v)+f_,-np.trace(dv)])
...
def eval_ins_base_handcode_bndstvc_intr_bndflux_pars_f(UQ,pars,x,n):
	nvar=UQ.shape[0]
	ndim=UQ.shape[1]-1
	Ub=UQ[:,0]
	dUb=np.zeros([nvar,nvar,ndim+1])
...
class IncompressibleStokes_f(object):
	...
	def __init__(self,ndim):
		...
		self.nvar=ndim+1
		self.srcflux=lambda UQ,f,pars,x:\
		             eval_ins_base_handcode_srcflux_stokes_f(UQ,f,pars,x)
		self.bndstvcflux=lambda nbcnbr,UQ,pars,x,n:\
					     eval_ins_base_handcode_bndstvc_intr_bndflux_pars_stokes_f(UQ,pars,x,n)

def eval_ins_base_handcode_srcflux_stokes_f(UQ,f,pars,x):
	u=UQ[:,0]; q=UQ[:,1:]
	ndim=u.shape[0]-1
	neqn=ndim+1
	ncomp=ndim+1
...
def eval_ins_base_handcode_bndstvc_intr_bndflux_pars_stokes_f(UQ,pars,x,n):
	nvar=UQ.shape[0]
	ndim=UQ.shape[1]-1
	Ub=UQ[:,0]
	dUb=np.zeros([nvar,nvar,ndim+1])
