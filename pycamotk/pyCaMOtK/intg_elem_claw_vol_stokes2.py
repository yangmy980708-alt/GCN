...
from source.FEM_ForwardModel import analyticalstokes2_f1, analyticalstokes2_f2
from source.TensorFEMCore_stokes2 import Double
...
def function_of_the_right_f(xq):
	nu = 1
	#xq=xq.reshape(-1,1)
	f1 = [Double(analyticalstokes2_f1(xq.reshape(-1, 1), nu).flatten().reshape(1, -1))]
	f2 = [Double(analyticalstokes2_f2(xq.reshape(-1, 1), nu).flatten().reshape(1, -1))]
	result1 = []
	for item in f1:
