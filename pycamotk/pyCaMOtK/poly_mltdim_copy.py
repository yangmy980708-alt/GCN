...
from pycamotk.pyCaMOtk.poly_onedim import eval_poly_nodal_onedim_lagr
from pycamotk.pyCaMOtk.tens_core import tensprod_scalar,      \
                               tensprod_scalar_unif, \
                               tensprod_vector_unif
...
def eval_monom_mltdim(coeffs, expnts, x):
    ...
    evaluation points.
    说明这个函数的功能是根据给定的系数、指数和高斯积分点来计算多项式及其导数的值。
    ...
    coeffs : ndarray (nb, npderiv)
      ...
      of ith monomial
      ndarray (nb, npderiv) 表示这是一个二维数组，其中 nb 是多项式的数量，npderiv 是每个多项式的导数数量。
      COEFFS[i, j] 表示第 i 个多项式第 j 个导数的系数
    ...
    for k in range(npderiv):
        for j in range(nb):
            ...
            Q[j, k, :] = coeffs[j, k]
            #将当前多项式的当前导数系数赋值给结果数组 Q 的相应位置（j 行，k 列），即设置当前多项式的导数初始值。
            for d in range(ndim):
                Q[j, k, :] *= x[d, :]**expnts[d, j, k]
                #将 x 在当前维度 d 的值（x[d, :]）的 expnts[d, j, k] 次方乘以当前多项式的值，以计算完整的多项式值。
    return Q

def eval_monom_mltdim_xk(coeffs, expnts, x):
    array = np.array(x)
    x = array.reshape(1,-1)
    nx = x.shape[1]
    ndim, nb, npderiv = expnts.shape
...
def eval_poly_nodal_mltdim_vander(xk, x, polysp='P', nderiv=0, glob_cont=0):
    ...
    # Extract information from input
    # nx = x.shape[1]#原程序
    # ndim, nv = xk.shape#原程序

    # x_shape = x.shape#用来验证下方的ndim nx 分别等于多少
    ndim, nx = x.shape#修改过后的nx  ndim
    nv = xk.size#修改过后的nv
    ...
    coeffsT, expntsT = get_monom_coeffs_expnts_mltdim(ndim, porder, polysp,
                                                      nderiv=nderiv)
    Vh = eval_monom_mltdim_xk(coeffsH, expntsH, xk)
    print('xk',xk)
    print('coeffsH',coeffsH)
    print('expntsH',expntsH)
    Vt = eval_monom_mltdim(coeffsT, expntsT, x)# xk传入该程序是会在247行出现超出索引的问题，因为xk没有第二个维度
    ...
    Q = np.linalg.solve(Vh, Vt)#Vh.Q=Vt
