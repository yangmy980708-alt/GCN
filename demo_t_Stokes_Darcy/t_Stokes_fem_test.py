...
MD=assemble_matrix_from_volume_integral_triangle(getattr(coef_data, 'function_one'),M_partition_D,T_partition_D,M_basis_phi,T_basis_phi,T_basis_phi,2,0,0,2,0,0);
...
MS=assemble_matrix_from_volume_integral_triangle(getattr(coef_data, 'function_one'),M_partition_S,T_partition_S,M_basis_u,T_basis_u,T_basis_u,2,0,0,2,0,0);
...
M[number_of_unknowns_Darcy:number_of_unknowns_Darcy+number_of_FE_nodes_u,number_of_unknowns_Darcy:number_of_unknowns_Darcy+number_of_FE_nodes_u]=MS;
M[number_of_unknowns_Darcy+number_of_FE_nodes_u:number_of_unknowns_Darcy+2*number_of_FE_nodes_u,number_of_unknowns_Darcy+number_of_FE_nodes_u:number_of_unknowns_Darcy+2*number_of_FE_nodes_u]=MS;
del MD,MS
...
X_old_Stokes=get_initial_vector_Stokes(getattr(coef_data, 'function_initial_u1'),getattr(coef_data, 'function_initial_u2'),
...
for n in range(int(N_t)):
    ...
    '''Darcy'''
    b1 = assemble_vector_from_volume_integral_time_triangle(getattr(coef_data, 'function_f_Poisson'),current_time,M_partition_D,T_partition_D,M_basis_phi,
                                                           T_basis_phi,2,0,0)
    b2 = assemble_vector_from_volume_integral_time_triangle(getattr(coef_data, 'function_f_Poisson'), current_time - dt, M_partition_D,T_partition_D,M_basis_phi,
                                                           T_basis_phi,2,0,0)
    b_Darcy = theta * b1 + (1 - theta) * b2

    '''Stokes'''
    b1 = assemble_vector_from_volume_integral_time_triangle(getattr(coef_data,'function_f1_Stokes'), current_time, M_partition_S,
                                                            T_partition_S, M_basis_u, T_basis_u, 2, 0, 0)
    b2 = assemble_vector_from_volume_integral_time_triangle(getattr(coef_data,'function_f1_Stokes'), current_time - dt, M_partition_S,
                                                            T_partition_S, M_basis_u, T_basis_u, 2, 0, 0)
    b3 = assemble_vector_from_volume_integral_time_triangle(getattr(coef_data,'function_f2_Stokes'), current_time, M_partition_S,
                                                            T_partition_S, M_basis_u, T_basis_u, 2, 0, 0)
    b4 = assemble_vector_from_volume_integral_time_triangle(getattr(coef_data,'function_f2_Stokes'), current_time - dt, M_partition_S,
                                                            T_partition_S, M_basis_u, T_basis_u, 2, 0, 0)
    ...
    [A_fixed, b] = treat_Dirichlet_boundary_time_triangle(getattr(coef_data, 'function_g_Poisson'), current_time, A_fixed, b,
                                                          boundary_nodes_D, M_basis_phi)

    if Dirichlet_switch == 1:
        [A_fixed, b] = treat_Dirichlet_boundary_time_Stokes_with_Darcy(getattr(coef_data, 'function_g1_Stokes'), getattr(coef_data, 'function_g2_Stokes'),
    ...
    fixed_p_index = 1
    [A_fixed, b] = fix_pressure_time_Stokes(getattr(coef_data, 'function_fix_p'), current_time, fix_pressure, A_fixed, b,
