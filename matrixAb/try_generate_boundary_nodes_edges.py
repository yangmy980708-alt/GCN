...
# Boundary nodes information matrix
nbn = 2 * (N1_basis + N2_basis)
boundary_nodes = np.zeros((2, nbn))

# Set all boundary nodes as Dirichlet (-1) by default
boundary_nodes[0, :] = -1

# Mark top boundary nodes as non-boundary (-100)
...
for k in range(N1_basis + N2_basis, 2 * N1_basis + N2_basis):
    boundary_nodes[1, k] = (2 * N1_basis + N2_basis + 1 - k) * (N2_basis + 1) - 1

# Left boundary nodes
for k in range(2 * N1_basis + N2_basis, nbn):
    boundary_nodes[1, k] = 2 * N1_basis + 2 * N2_basis - k

# Boundary edges information matrix
nbe = 2 * (N1_partition + N2_partition)
boundary_edges = np.zeros((4, nbe))

# Set all boundary edges as Dirichlet (-1) by default
boundary_edges[0, :] = -1

# Mark top boundary edges as non-boundary (-100)
...
for k in range(N1_partition, N1_partition + N2_partition):
    ...
    boundary_edges[3, k] = N1_partition * (N2_partition + 1) + (k - N1_partition + 2)-1
...
print('nodes=',boundary_nodes)
print('edges=',boundary_edges)