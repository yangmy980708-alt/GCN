...
class create_ldof2gdof_cg_mixed2(object):
	...
	def __init__(self,ndof_per_node1,e2vcg1,
		...
		base_gdof_u1 = self.e2vcg1
		base_gdof_u2 = self.e2vcg1 + np.max(e2vcg1) + 1
		gdof1 = np.vstack((base_gdof_u1, base_gdof_u2))
		gdof2= e2vcg2 + (np.max(e2vcg1)+1) * self.ndof_per_node1
		# gdof1=gdof1.reshape(-1,1,order='F')
		# gdof2=gdof2.reshape(-1,1,order='F')
		# gdof=np.vstack((gdof1, gdof2))

		self.ldof2gdof=np.vstack((gdof1, gdof2))
